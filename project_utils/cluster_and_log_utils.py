...
def split_cluster_acc_v1(y_true, y_pred, mask):
    ...
    mask = mask.astype(bool) # Mask corresponds to 'Old' classes
...
def log_accs_from_preds(y_true, y_pred, mask, eval_funcs: List[str], save_name: str, T: int=None,
                        print_output=True,num_round = None):
    ...
    for i, f_name in enumerate(eval_funcs):
        ...
        if i == 0:
            ...
            to_return2 = (all_acc, old_acc, new_acc)
        if print_output:
            print_str = f'Epoch {T}, {log_name}: All {all_acc:.4f} | Old {old_acc:.4f} | New {new_acc:.4f}'
            if num_round is not None:
                print_str = f'Round {num_round}, Epoch {T}, {log_name}: All {all_acc:.4f} | Old {old_acc:.4f} | New {new_acc:.4f}'
            print(print_str)
