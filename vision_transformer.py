...
class SelfAttention(nn.Module):
    def __init__(self, in_dim, dropout_rate=0.1):
        ...
        self.dropout = nn.Dropout(dropout_rate)  # Adding the dropout layer
    ...
    def forward(self, x):
        ...
        attention_scores = F.softmax(torch.matmul(Q, K.transpose(-2, -1)) / (K.size(-1) ** 0.5), dim=-1)
        attention_output = torch.matmul(attention_scores, V)
        return attention_output

class HASHHead(nn.Module):
    def __init__(self, args, in_dim, out_dim, use_bn=False, norm_last_layer=True, nlayers=3, hidden_dim=2048, bottleneck_dim=256, code_dim=32, class_num=200):
        super().__init__()
        ...
        ### Modified
        self.doublehash = args.double

        self.bn = nn.BatchNorm1d(bottleneck_dim)
        self.attention = SelfAttention(bottleneck_dim)
        self.linear = nn.Linear(bottleneck_dim, bottleneck_dim)
        self.bn2 = nn.BatchNorm1d(bottleneck_dim)

        self.hash2 = nn.Linear(bottleneck_dim, code_dim, bias=False)
        self.variance2 = nn.Linear(bottleneck_dim, code_dim, bias=False)
        self.bn_h2 = nn.BatchNorm1d(code_dim)
        self.bn_v2 = nn.BatchNorm1d(code_dim)
...
class DINOHead(nn.Module):
    ...
    def forward(self, x):
        x = self.mlp(x)
        h = nn.functional.normalize(x, dim=-1, p=2)
        x = self.last_layer(h)
        return h,x
